...
class _MapGoogleState extends ConsumerState<MapGoogle> {
  ...
  final double _geoFenceRadius = 200; // meters
...
// Show location info (placeholder)
  ...
  Widget build(BuildContext context) {
    // final circles = ref.watch(geofenceCircleProvider);
  ...
  Widget _buildMap(LatLng? location, Set<Marker> markers, MapType mapType,
    ...
    return GoogleMap(
      ...
      initialCameraPosition: CameraPosition(
        target: location ?? const LatLng(37.7749, -122.4194),
        zoom: 15.0,
      ),
      markers: markers,
  ...
  void _addPolylineToMap(Polyline polyline) {
    ...
    final updatedPolylines = Map<PolylineId, Polyline>.from(currentPolylines);
    updatedPolylines[polyline.polylineId] = polyline;
    ref.read(polylinesProvider.notifier).state = updatedPolylines;
  }
...
//   ]);
// }
// Future<void> _calculateAndDisplayRoute(LatLng origin, LatLng destination) async {
//   int retryCount = 0;
//   const int maxRetries = 3;
//
//   while (retryCount < maxRetries) {
...
//       retryCount++;
//       if (retryCount >= maxRetries) {
//         _showErrorSnackBar('Failed to calculate route after $maxRetries attempts: $e');
//         _clearRoute();
//         return;
//       }
//
